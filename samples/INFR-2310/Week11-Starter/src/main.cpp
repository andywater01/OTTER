...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "100744494_Andrew_Waterhouse", nullptr, nullptr);
...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->LoadShaderPartFromFile("shaders/Terrain.vert.glsl", GL_VERTEX_SHADER);
		shader->LoadShaderPartFromFile("shaders/Terrain.frag.glsl", GL_FRAGMENT_SHADER);
		shader->Link();
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::SliderFloat3("Interpolation Factors", &interpolateFactors[0], 0, 1))
			{
				shader->SetUniform("u_InterpolateFactors", interpolateFactors);
			}
		...
		Texture2D::sptr rock = Texture2D::LoadFromFile("images/stone.jpg");
		...
		GameObject obj5 = scene->CreateEntity("cube");
		{
			...
			obj5.get<Transform>().SetLocalPosition(0.0f, 0.0f, -0.1f).SetLocalScale(glm::vec3(20));
